...
<a class="w-full py-2 flex  hover:bg-gray-100 feedback-container rounded mb-1
    ...
    <div class="w-full flex items-center justify-between px-4" id="feedback-content-{{$feedback->id}}">
        <div class="w-full md:w-1/4" id="feedback-name-{{$feedback->id}}">
            <h2 class="poppins text-base font-medium
            @if($feedback->isRead) text-gray-500 @else text-gray-900  @endif">{{$feedback->name}}</h2>
        </div>

        <div class="hidden md:block w-1/2 overflow-hidden" id="feedback-message-{{$feedback->id}}">
        ...
        <div class="w-full md:w-1/4 flex justify-end" id="feedback-date-{{$feedback->id}}">
