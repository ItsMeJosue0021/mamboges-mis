    ...
    <div class="flex w-full absolute top-0 left-0 pt-40 items-center justify-center">
        <div class="w-96 h-fit rounded-md bg-white shadow-2xl bg-gradient-to-b from-gray-100 to-white">
            <form class="flex flex-col space-y-3 py-4 px-6"  method="post" action="{{route('activity.store')}}"> {{--action="{{route('activity.store')}}"--}}
                @csrf
                <div>
                    <h1 class="poppins font-semibold">NEW PERFORMANCE TASK</h1>
                </div>
                ...
                <div class="flex flex-col space-y-1">
                 ...
                 <input type="hidden" name="evaluation_criteria_id" value="{{ $evaluations->skip(1)->first()->id }}"> {{--data-evaluation-id="{{ $evaluations->first()->id }}" --}}
                <div class="flex items-center justify-end space-x-4 pt-2">
                    ...
                    <a id="pt-act-cancel" class="poppins cursor-pointer px-6 py-2 hover:bg-gray-200 rounded">Cancel</a>
                </div>
            </form>
        </div>
    </div>  
