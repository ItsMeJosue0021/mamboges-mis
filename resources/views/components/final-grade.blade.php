<div id="final" class="hidden">
            ...
            <div class="w-full flex items-center justify-between space-x-4">
                ...
                <div class="flex items-center space-x-4">
                    <a href="{{ route('class.record.printable', $class->id) }}" class="text-sm bg-gray-600 hover:bg-gray-700 text-white py-2 px-4 flex items-center poppins">
                        <i class='bx bx-printer text-white text-sm mr-2'></i> Print
                    </a>
                    @if ($classrecord->is_released)
                        <button id="unreleaseGradeButton" onclick="unrelease()" class="text-sm bg-red-600 hover:bg-red-700 text-white py-2 px-4 flex items-center poppins">Unrelease</button>
                    @else
                        <button id="releaseGradeButton" onclick="release()" class="text-sm bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 flex items-center poppins">Release</button>
                    @endif
                </div>
            </div>
        </div>
    ...
    <form id="unreleaseGradeForm" action="{{ route('grade.unrelease', $class->id) }}" method="POST">
        @csrf
        <input hidden name="section" value="{{ $class->section->id }}">
        <input hidden name="class_record" value="{{ $classrecord->id }}">
        <input hidden name="quarter" value="{{ $classrecord->quarter->id }}">

        <input hidden name="subject" value="{{ $classrecord->sectionSubjects->subjects->id }}">
        <input hidden name="school_year" value="{{ $schoolyear->id }}">
    </form>

    {{-- class record to be printed --}}
    <form id="submitGradeForm" action="{{ route('grade.release', $class->id) }}" method="POST">
        @csrf
    ...
    <script>
        ...
        function release() {
            const releaseGradeButton = document.querySelector('#releaseGradeButton');
            const submitGradeForm = document.querySelector('#submitGradeForm');

            releaseGradeButton.addEventListener('click', () => {
                submitGradeForm.submit();
            });
        }

        function unrelease() {
            const unreleaseGradeButton = document.querySelector('#unreleaseGradeButton');
            const unreleaseGradeForm = document.querySelector('#unreleaseGradeForm');

            unreleaseGradeButton.addEventListener('click', () => {
                unreleaseGradeForm.submit();
            });
        }
