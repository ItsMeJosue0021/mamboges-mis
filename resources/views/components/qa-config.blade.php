    ...
    <div class="w-full flex items-center space-x-4 rounded">
        <div class="w-fit flex items-center space-x-4">
            ...
            <div class="flex items-center space-x-2">
                <form id="QA_PercentageForm"  method="POST" action="javascript::void(0)" class="flex items-center rounded py-2 px-2 border border-gray-300">
                    @csrf
                    @method('PUT')
                    ...
                    <button id="QA_changePercentageButton" data-evaluation-id="{{ $evaluations->skip(2)->first()->id }}"  class="poppins text-xs text-white bg-sky-600 hover:bg-sky-700 px-2 py-1 font-bold rounded-[3px]">CHANGE</button>
                </form>
            </div>
        </div>
        ...
        <div class="w-fit flex items-center space-x-3 rounded py-2 px-2 border border-gray-300">
            ...
            <a id="new-assessment" class="poppins text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 px-2 py-1 rounded-[3px] cursor-pointer">NEW</a>
        </div>
    </div>
    ...
    $(document).ready(function() {
        ...
        function getPercentage(criteria) {
            $.ajax({
                ...
                success: function(response) {
                    if (response.status === 'success') {
                        $('#percentageValue_2').text(response.percentage + '%');
